...
omm format <element>               # 다이어그램 형식 표시 (mermaid/plantuml)
omm config plantuml-status         # PlantUML 상태 확인
...
여러 프로젝트가 포함된 아키텍처 저장소에서 `omm view`를 실행하면 프로젝트 선택기가 자동으로 표시됩니다. `--project <이름>`을 수동으로 지정할 필요가 없습니다.

선택기에는 다음 정보가 포함된 카드로 모든 프로젝트가 나열됩니다:
...
2개 이상의 조직이 구성된 경우 상단에 org 전환기가 표시되어 조직 간에 쉽게 전환할 수 있습니다.
